...
function prepareCopilotRipgrepShimTask(platform: string, arch: string, destinationFolderName: string) {
	...
	return async () => {
		...
		const sourceCopilotSdkDir = path.join(root, 'extensions', 'copilot', 'node_modules', '@github', 'copilot', 'sdk');
		prepareBuiltInCopilotRipgrepShim(platform, arch, builtInCopilotExtensionDir, appNodeModulesDir, sourceCopilotSdkDir);
	};
}
